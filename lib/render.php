    ...
    foreach ($_SESSION['__v:flash']['messages'] as $f) {
      if (!strlen($which) || ($f['which'] == $which)) {
        if ((strlen($which) || $is_flash_tag || !$render_context->get("has_flash_tag" . $f['which'])) && !$_VAE['flash_rendered'][$f['which']]) {
          if ((!strlen($f['which']) || !in_array($f['which'], $skip)) && !$shown[$f['msg']]) {
...
function _vae_render_img($a, &$tag, $context, &$callback, $render_context) {
  ...
  if ($a['path']) {
    ...
    if (isset($a['grow']) && !isset($a['size_method'])) $a['size_method'] = "grow";
    $a['src'] = (($a['image_size'] && !$a['width']) ? vae_sizedimage(_vae_fetch($a['path'], $context), $a['image_size'], $preserve_filename) : vae_image(_vae_fetch($a['path'], $context), $a['width'], $a['height'], $a['image_size'], $a['size_method'], $a['quality'], $preserve_filename, $a['trim']));
    if (!$a['src']) return "";
...
function _vae_render_pagination($a, &$tag, $context, &$callback, $render_context) {
  ...
  for ($i = $start_page; $i <= $end_page; $i++) {
    $data = '<a class="' . $class . (($b['page'] == $i) ? ' current' : '') . '" href="' . htmlspecialchars($_SERVER['PHP_SELF']) . _vae_qs(array($b['page_request_variable'] => $i)) . '">' . $i . '</a>';
