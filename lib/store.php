...
function _vae_store_callback_address_delete($tag) {
  $id = $tag['callback']['id'];
  if (is_numeric($id)) {
    $raw = _vae_rest(array(), "customer_addresses/destroy/" . $id, "customer_address", $tag, null, true);
...
function _vae_store_callback_paypal_express_checkout($tag, $from_select = false) {
  ...
  if ($_REQUEST['token']) {
    ...
    if ($url = _vae_rest(array('total' => _vae_store_compute_subtotal(), 'ip' => _vae_remote_addr(), 'return_url' => $return_url, 'cancel_return_url' => $cancel_url), "store/paypal_express_checkout", "order")) {
...
function _vae_store_compute_discount($item = null, $remaining = null, $flash_location = '') {
  $amount = 0;
...
function _vae_store_item_available($item, $options_collection, $inventory_field) {
  global $_VAE;
  $root_inv = _vae_fetch_without_errors($inventory_field, $item);
