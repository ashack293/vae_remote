...
function _vae_store_callback_paypal_express_checkout($tag, $from_select = false) {
  ...
  if ($_REQUEST['token']) {
    ...
    return vae_redirect($_SERVER['PHP_SELF']);
  } elseif ($_REQUEST['__cancel_paypal']) {
    $_SESSION['__v:store']['payment_method'] = "";
    return vae_redirect($_SERVER['PHP_SELF']);
  } else {
    ...
    $cancel_url = _vae_proto() . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . ($from_select ? _vae_qs("__v:store_payment_methods_select=&__method=") : _vae_qs("__cancel_paypal=1&__v:store_paypal_express_checkout=" . _vae_tag_unique_id($tag, $context)));
    $options = array('total' => _vae_store_compute_total(), 'ip' => _vae_remote_addr(), 'return_url' => $return_url, 'cancel_return_url' => $cancel_url);
    foreach (array('subscription_description','skip_registration') as $k) {
...
function _vae_store_payment_paypal_ipn() {
  ...
  _vae_log($out);
  if (!$good) {
    if ($report_error) _vae_log_error("A callback received from PayPal could not be processed.\n\nDebugging information follows:\n\n" . $out, "PayPal IPN Error", debug_backtrace());
