...
define('MODULE_SHIPPING_CONFIG_DMSTC_FIRSTCLASS_THRESHOLD', '0, 3.5, 3.5, 10, 10, 13, 0, 13');
	...
	$rates = array();
      ...
      if ($order->delivery['country']['id'] == SHIPPING_ORIGIN_COUNTRY) {
        ...
        if (ereg('<Error>', $response[0])) {
		  ...
		  $service = $regs[1];
		  $service = html_entity_decode(str_replace("*", "", trim(str_replace(array("&reg;", "&#8482;"), "", strip_tags($service)))));
