...
function _vae_imagesize($d, $complain = false) {
  ...
  if (!strlen($d)) return;
  $iden = $d."size";
  if (isset($_VAE['file_cache'][$iden])) return explode(",", $_VAE['file_cache'][$iden]);
  $tk = _vae_gd_handle($d);
  if ($tk == null) {
...
function vae_text($text, $font_name = "", $font_size = "22", $color = "#000000", $kerning = 1, $padding = 5, $max_width = 10000) {
  ...
  _vae_load_cache();
  if (false && isset($_VAE['file_cache'][$iden])) {
    $file_name = $_VAE['file_cache'][$iden];
  } else {
